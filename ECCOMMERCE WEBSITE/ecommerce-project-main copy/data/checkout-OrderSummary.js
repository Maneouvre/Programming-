...
async function displayOrderSummary() {
  await loadProducts();
  await renderOrderSummary(); // Added await here since renderOrderSummary is async
}
displayOrderSummary();

export async function renderOrderSummary(productsList) {
  if (!productsList) {
  ...
  // Use for...of instead of cart.forEach to support await
  for (const item of cart) {
    const productId = item.productId;
    
    //Added await here so it resolves the promise to an actual product
    ...
    `;
  } // FIXED: Properly closed the for...of loop here
  ...
  document.querySelectorAll('.js-delivery-option').forEach((element) => {
    element.addEventListener('click', async () => { // Added async
      const productId = element.dataset.productId;
      const deliveryOptionId = element.dataset.deliveryOptionId;
      updateDeliveryOption(productId, deliveryOptionId);
      await renderOrderSummary(); // Added await to reload cleanly
      renderPaymentSummary();
    });
  });
}
